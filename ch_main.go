...
func init() {
	...
	flag.StringVar(&chHost, "chHost", "localhost", "ClickHouse host")
	flag.StringVar(&chPort, "chPort", "9000", "ClickHouse native protocol port")
	flag.StringVar(&chDatabase, "chDatabase", "default", "ClickHouse database name")
	flag.StringVar(&chUser, "chUser", "default", "ClickHouse username")
	flag.StringVar(&chPassword, "chPassword", "", "ClickHouse password")
...
func initDB() (*sql.DB, error) {
	...
	CREATE TABLE IF NOT EXISTS tb_cdr (
		...
		T38Rx String,
		T38Tx String,
		ErrorFromNetwork String,
		ErrorToNetwork String,
		MOS String,
		NetworkQuality String,
		InsertTime DateTime DEFAULT now()
	) ENGINE = MergeTree()
	PARTITION BY toYYYYMM(Timestamp)
	ORDER BY (Timestamp, SessionID, LegID)
	`
...
func main() {
	log.Info("Starting CDR processor for ClickHouse")
	...
	log.Info("ClickHouse connection initialized")
	...
	go watchDirectory(mainDir, watcher)
...
func dbPush(cdrs []CDR) error {
	...
	// ClickHouse performs better with batch inserts
	tx, err := sqlDB.Begin()
	if err != nil {
	...
	for _, cdr := range cdrs {
		if !debug {
			...
			if err != nil {
				log.Errorf("Error inserting CDR record: %v", err)
				return err
			}
		} else {
	...
	log.Infof("Successfully inserted %d CDR records", len(cdrs))
	return nil
}
...
func extractGzip(src, destDir string) error {
	...
	// Push CDR records to the database
	err = dbPush(cdrs)
	if err != nil {
	...
	if err = os.Remove(extractedFilePath); err != nil {
		log.Errorf("Error removing extracted file: %v", err)
	}
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "BEG") {
			...
			if len(match) > 0 {
				...
				// Format the time in a standard format for ClickHouse
			...
			log.Debugf("Unknown CDR type. Skipping line: %s", line)
			continue
		}
		...
		if len(match) > 0 {
			...
			log.Debugf("Line did not match. Skipping line: %s", line)
		}
